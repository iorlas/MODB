...
if len(sys.argv) > 1 and sys.argv[1] == '-h':
    print """This file imports WORLD and SCRIPTS databases.
    Why one separated script for only this two databases?
    Its can be imprted any time by any server w/o migration.
    This databases are untouchable for server and cannot keep something, what you are need to keep.
    """
...
#get list of dumps
try:
    world_dumps_list = os.listdir(rel('world'))
except:
    print "WARNING!!!\nIt seems, you have no \"world\" directory :("
    exit(1)
try:
    scripts_dumps_list = os.listdir(rel('scripts'))
except:
    print "WARNING!!!\nIt seems, you have no \"scripts\" directory :("
    exit(1)
